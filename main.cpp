...
std::string fabricatePath(const std::string &user,
  ...
  const std::vector<std::string> bases = {"/home", "/opt", "/var", "/usr/local",
                                          "/srv", "/mnt"};
  std::uniform_int_distribution<int> baseDist(0,
                                              static_cast<int>(bases.size()) - 1);
...
std::string pickInstallPath(const std::vector<std::string> &realPaths,
  ...
  if (!realPaths.empty() && chance(rng) < 35) {
    std::uniform_int_distribution<int> idx(0,
                                           static_cast<int>(realPaths.size()) -
                                               1);
...
bool runFakeInstaller(std::mt19937 &rng) {
  ...
  std::vector<std::string> realPaths =
      collectRealPaths(std::getenv("USER") ? std::getenv("USER") : "root");
  const std::vector<int> actions = {0, 0, 1, 1, 1, 2, 2, 2, 3};  
    ...
    switch (actions[static_cast<size_t>(actionDist(rng))]) {
      ...
      case 2: {
        ...
        const std::string path =
            pickInstallPath(realPaths,
                            std::getenv("USER") ? std::getenv("USER") : "root",
                            items, rng);
